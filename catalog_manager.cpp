#include "header/catalog_manager.h"
...
void CatalogManager::createTable(std::string table_name, Attribute attribute, int primary, Index index)
{
    if (hasTable(table_name))
        throw table_exist();          // 抛出同名表已经存在的异常
    ...
    if (blockNum != 0)
    {
        for (int blockNo = 0; blockNo < blockNum; blockNo++)
        {
            ...
            if (length + tb_info_len_int >= PAGESIZE)
                continue; // 当前块空间不足，继续循环
...
Attribute CatalogManager::getAttribute(std::string table_name)
{
    ...
    for (int idx = 0; idx < 32; idx++)
        ret.has_index[idx] = false;
    for (int idx = 0; idx < index_record.num; idx++)
        ret.has_index[index_record.location[idx]] = true;
...
void CatalogManager::createIndex(std::string table_name, std::string attr_name, std::string index_name)
{
    ...
    {
        // 当前属性上已经有索引的异常
        if (attr.name[index.location[idx]] == attr_name)
            throw index_exist();
