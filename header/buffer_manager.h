...
class Page {
 public:
  ...
  int getBlockId();
  void setUseCount(int _use_count);
  int getUseCount();
 ...
 private:
  ...
  int use_count;  //记录被钉住的次数。被钉住的意思就是不可以被替换
...
class BufferManager {
 ...
 private:
  Page*
      frames;  //缓冲池，实际上就是一个元素为Page的数组，实际内存空间将分配在堆上
  ...
  __gnu_pbds::priority_queue<Page, std::less<Page>,
