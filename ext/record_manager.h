...
public:
  ...
  int deleteRecord(std::string table_name , std::vector<std::string> target_attr , vector<Where> where);
  //输入：表名
  //输出：Table类型对象
  //功能：返回整张表
  //异常：如果表不存在，抛出table_not_exist异常
...
private:
  ...
  //在块中进行条件删除
  int conditionDeleteInBlock(std::string table_name , int block_id , Attribute attr , std::vector<int> index , std::vector<Where> where);
  //在块中进行条件查询
