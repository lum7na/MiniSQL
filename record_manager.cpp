#include "header/record_manager.h"
...
int RecordManager::deleteRecord(std::string table_name, std::string target_attr, Where where)
{
    ...
    if (attr.type[index] != where.data.type)
    ...
    if (attr.has_index[index] && where.relation_character != NOT_EQUAL)
    {
        ...
        for (int idx = 0; idx < block_ids.size(); idx++)
            ret += conditionDeleteInBlock(tmp_name, block_ids[idx], attr, index, where);
    }
    else
        for (int idx = 0; idx < block_num; idx++)
            ret += conditionDeleteInBlock(tmp_name, idx, attr, index, where);
...
Table RecordManager::selectRecord(std::string table_name, std::string target_attr, Where where, std::string result_table_name)
{
    ...
    if (attr.has_index[index] && where.relation_character != NOT_EQUAL)
    {
        ...
        searchWithIndex(tmp_name, target_attr, where, block_ids);
        for (int idx = 0; idx < block_ids.size(); idx++)
            conditionSelectInBlock(tmp_name, block_ids[idx], attr, index, where, tuples);
    }
    else
    {
        int block_num = getBlockNum(table_name);
        for (int idx = 0; idx < block_num; idx++)
            conditionSelectInBlock(tmp_name, idx, attr, index, where, tuples);
    }
...
void RecordManager::insertRecord1(char *p, int offset, int len, const std::vector<Data> &v)
{
    ...
    for (int idx = 0; idx < v.size(); idx++)
    {
        ...
        switch (d.type)
        {
            case -1: copyString(p, offset, d.datai); break;
            case  0: copyString(p, offset, d.dataf); break;
            default: copyString(p, offset, d.datas); break;
        }
    }
    p[offset++] = ' ';
...
bool RecordManager::isConflict(std::vector<Tuple> &tuples, std::vector<Data> &v, int index)
{
    ...
    {
        // 若已经标记删除，则继续
        if (tuples[idx].isDeleted())
            continue;
        
        std::vector<Data> tuple_data = tuples[idx].getData();
        switch (v[index].type)
        {
            case -1: if (v[index].datai == tuple_data[index].datai) ret = true; break;
            case  0: if (v[index].dataf == tuple_data[index].dataf) ret = true; break;
            default: if (v[index].datas == tuple_data[index].datas) ret = true; break;
        }
    }
...
void RecordManager::searchWithIndex(std::string table_name, std::string target_attr, Where where, std::vector<int> &block_ids)
{
    ...
    if (where.relation_character == LESS || where.relation_character == LESS_OR_EQUAL)
    {
        ...
        {
            case -1: tmp_data.type = -1; tmp_data.datai = -INF; break;
            case  0: tmp_data.type =  0; tmp_data.dataf = -INF; break;
            default: tmp_data.type =  1; tmp_data.datas =   ""; break;
        }
    ...
    }else if (where.relation_character == GREATER || where.relation_character == GREATER_OR_EQUAL)
    {
        ...
        switch (where.data.type)
        {
            case -1: tmp_data.type = -1; tmp_data.datai =  INF; break;
            case  0: tmp_data.type =  0; tmp_data.dataf =  INF; break;
            default: tmp_data.type = -2; tmp_data.datas =   ""; break;
        }
